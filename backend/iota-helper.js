...
async function  generateAddress(seed){
    
    let address1 = await iota.getNewAddress(seed,{checksum:true});
...
async function getBalance(seed){
    ...
    let balance = await iota.getAccountData(seed,{start: 0,security: 2})
    return balance;
}
